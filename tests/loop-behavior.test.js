  ...
  it('resolves completion review requests before signaling the worker', async () => {
    ...
    try {
      ...
      fs.writeFileSync(cliPath, [
        ...
        '  // Workers can\'t see coord/requests.jsonl through the per-worker view,',
        '  // but decisions.jsonl is exposed (read-only via symlink). A decision',
        '  // appears here only after processApprovals flips the request status to',
        '  // resolved/rejected, so a hit here proves the request was already',
        '  // resolved before this SIGTERM fired.',
        '  try {',
        '    const lines = fs.readFileSync(path.join("coord", "decisions.jsonl"), "utf-8").trim().split(/\\n/).filter(Boolean);',
        '    const decisions = lines.map((line) => JSON.parse(line));',
        '    const decision = decisions.find((entry) => entry.request_id === agentName + "-done");',
        '    if (!decision) return "missing";',
        '    return decision.disposition === "approved" ? "resolved" : decision.disposition;',
        '  } catch (err) {',
